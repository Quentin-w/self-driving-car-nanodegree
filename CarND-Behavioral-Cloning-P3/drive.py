...
def telemetry(sid, data):
    if data:
        ...
        image = Image.open(BytesIO(base64.b64decode(imgString)))#img in RGB
        image_array = np.asarray(image)
    #########################################################################
        image_array = process_image(image_array)#process YUV images
    #########################################################################    
...
def send_control(steering_angle, throttle):
    sio.emit('steer', data={'steering_angle': steering_angle.__str__(),
             'throttle': throttle.__str__()}, skip_sid=True)
